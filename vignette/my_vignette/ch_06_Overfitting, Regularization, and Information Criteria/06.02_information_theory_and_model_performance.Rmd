...
We can use a model's deviance to approximate the model's divergence.
```{r}
(DIVERGENCE <- (-2) * sum(log(q))) %>% round(2)
```

In this case our model's predicted distribution of `r q` has a divergence of `r DIVERGENCE`.

*Divergence should have been based on all observations rather than the 2 values of the model q, the values of q would accompany each observed outcome*